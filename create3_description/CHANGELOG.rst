^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package create3_description
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

3.1.0 (2026-02-27)
------------------
* Removed all Gazebo/simulation-specific elements, including plugins, conditionals, and arguments from URDF/Xacro files to focus on real hardware support.
* Simplified macros (e.g., bumper, caster, sensors) by removing Gazebo parameters.
* Added optional `use_inertia` argument (default: false) for conditional inclusion of root link inertia to avoid KDL warnings in non-simulation environments.
* Changed package name to fit the create3_examples repo
* Contributors: scottcandy34
