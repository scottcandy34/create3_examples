^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package create3_common_bringup
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

3.1.0 (2026-02-27)
------------------
* Fixed deprecation warnings for static_transform_publisher by using new named-argument style (e.g., --x, --y) in launch files.
* Removed Gazebo/simulation-specific arguments from launch configurations to align with hardware-focused updates.
* Add wheel-drop static transforms
